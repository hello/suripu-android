...
public class AccountInteractor extends ValueInteractor<Account> {
    ...
    public
    @Inject
    AccountInteractor(@NonNull Context context) {
    ...
    public Observable<Void> pullAccountPreferences() {
        ...
        return preferences().map(prefs -> {
            preferences.edit()
                       .putBoolean(PreferencesInteractor.PUSH_SCORE_ENABLED, prefs.pushScore)
                       .putBoolean(PreferencesInteractor.PUSH_ALERT_CONDITIONS_ENABLED, prefs.pushAlertConditions)
                       .putBoolean(PreferencesInteractor.ENHANCED_AUDIO_ENABLED, prefs.enhancedAudioEnabled)
                       .putBoolean(PreferencesInteractor.USE_CELSIUS, prefs.useCelsius)
                       .putBoolean(PreferencesInteractor.USE_GRAMS, prefs.useMetricWeight)
                       .putBoolean(PreferencesInteractor.USE_CENTIMETERS, prefs.useMetricHeight)
                       .putBoolean(PreferencesInteractor.USE_24_TIME, prefs.use24Time)
                       .apply();
    ...
    public void pushAccountPreferences() {
        ...
        update.pushScore = preferences.getBoolean(PreferencesInteractor.PUSH_SCORE_ENABLED, true);
        update.pushAlertConditions = preferences.getBoolean(PreferencesInteractor.PUSH_ALERT_CONDITIONS_ENABLED, true);
        update.enhancedAudioEnabled = preferences.getBoolean(PreferencesInteractor.ENHANCED_AUDIO_ENABLED, false);
        update.use24Time = preferences.getUse24Time();
        update.useCelsius = preferences.getBoolean(PreferencesInteractor.USE_CELSIUS, defaultMetric);
        update.useMetricWeight = preferences.getBoolean(PreferencesInteractor.USE_GRAMS, defaultMetric);
        update.useMetricHeight = preferences.getBoolean(PreferencesInteractor.USE_CENTIMETERS, defaultMetric);
