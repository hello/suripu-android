...
public class SensorResponseAdapter extends ArrayRecyclerAdapter<Sensor, SensorResponseAdapter.BaseViewHolder> {
    ...
    private class AirQualityViewHolder extends SensorBaseViewHolder {
        ...
        @Override
        public void bind(final int position) {
            final AirQualityCard airQualityCard = new AirQualityCard(this.view.getContext());
            airQualityCard.setUnitFormatter(unitFormatter);
            airQualityCard.replaceAll(airQualitySensors);
            this.body.setText(airQualityCard.getWorstMessage());
            if (this.root.getChildCount() < 3) {
                this.root.addView(airQualityCard);
            }
        }
    }
