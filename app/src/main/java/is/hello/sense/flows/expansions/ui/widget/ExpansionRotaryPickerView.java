...
/**
 * Copied {@link is.hello.sense.ui.widget.RotaryPickerView} to not waste time supporting different
 * styles. //todo merge both and support different styles.
 */
public class ExpansionRotaryPickerView extends RecyclerView implements View.OnClickListener {
    ...
    public ExpansionRotaryPickerView(@NonNull final Context context) {
        this(context, null);
    }

    public ExpansionRotaryPickerView(@NonNull final Context context, @Nullable final AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ExpansionRotaryPickerView(@NonNull final Context context, @Nullable final AttributeSet attrs, final int defStyle) {
    ...
    @Override
    protected void onMeasure(final int widthSpec,
                             final int heightSpec) {

        this.itemWidth = (longestValueStringLength * glyphWidth) + (itemHorizontalPadding * 2);
        this.itemHeight = glyphHeight + itemVerticalPadding;
    ...
    public void setSymbol(@NonNull final String symbol) {
        if (Constants.EMPTY_STRING.equals(symbol)) {
            if (!Constants.EMPTY_STRING.equals(this.symbol)) {
                this.longestValueStringLength--;
            }
        } else if (Constants.EMPTY_STRING.equals(this.symbol)) {
            this.longestValueStringLength++;
        }
        this.symbol = symbol;
    ...
    class ScrollListener extends OnScrollListener {
        ...
        private void updateValueFromView(@NonNull View view) {
            ...
            ExpansionRotaryPickerView.this.value = newValue;
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            for (int i = recyclerView.getChildCount() - 1; i >= 0; i--) {
                ...
                final ViewHolder viewHolder = recyclerView.getChildViewHolder(view);
            ...
            if (wrapAroundEventsEnabled && wrapsAround && onSelectionListener != null) {
                ...
                if (dy > 0f) {
                    ...
                    if (position == rollForwardPosition) {
                        onSelectionListener.onSelectionRolledOver(ExpansionRotaryPickerView.this,
                                                                  RolloverDirection.FORWARD);
                        ExpansionRotaryPickerView.this.lastWrapAroundPosition = adapterPosition;
                        return;
                    }
                } else {
                    final int position = adapterPosition % adapter.getBoundedItemCount();
                ...
                ExpansionRotaryPickerView.this.lastWrapAroundPosition = NO_POSITION;
            }
        }
            ...
            if (previousState == SCROLL_STATE_IDLE && newState == SCROLL_STATE_DRAGGING) {
                ...
                if (previousState != SCROLL_STATE_IDLE && newState == SCROLL_STATE_IDLE) {
                    ...
                    if (distanceToNotch == 0) {
                        ExpansionRotaryPickerView.this.wrapAroundEventsEnabled = true;
    ...
    class Decoration extends ItemDecoration {
        @Override
    ...
    class ItemAdapter extends Adapter<ItemAdapter.ItemViewHolder> {
        final float UNFOCUSED_SCALE = .5f, FOCUSED_SCALE = 1f;
        final
        @ColorInt
        int focusedColor, unfocusedColor, almostGoneColor;
        ...
        public String getItemString(int position) {
            if (valueStrings != null) {
                ...
                if (Constants.EMPTY_STRING.equals(symbol)) {
                    ...
                    return String.format("%02d%s", getItem(position), symbol);
                }
            } else {
                if (Constants.EMPTY_STRING.equals(symbol)) {
                    return Integer.toString(getItem(position));
                } else {
                    return String.format("%d%s", getItem(position), symbol);
                }
            }
        }

        @Override
        public ItemViewHolder onCreateViewHolder(final ViewGroup parent,
                                                 final int viewType) {
            ...
            itemView.setOnClickListener(ExpansionRotaryPickerView.this);
    ...
    public interface OnSelectionListener {
        void onSelectionRolledOver(@NonNull ExpansionRotaryPickerView picker, @NonNull RolloverDirection direction);
