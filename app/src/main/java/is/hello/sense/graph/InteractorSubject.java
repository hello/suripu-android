 ...
 * This means that calls to {@see InteractorSubject.onCompleted} do nothing, and that
 * the subject will continue to emit values even after {@see InteractorSubject.onError}
 * has been called. This subject is intended to hold volatile in-memory state for subclasses
 * of the {@see is.hello.sense.interactors.Interactor} class.
 * <p/>
 * InteractorSubject retains, at most, a single value/error in memory at a time.
...
public final class InteractorSubject<T> extends Subject<T, T> {
    ...
    public static <T> InteractorSubject<T> create() {
        ...
        return new InteractorSubject<>(subscriptionManager);
    }
     ...
     * exclusively inside of InteractorSubject objects,
     ...
     * <p/>
     * InteractorSubject can be updated from a background thread,
